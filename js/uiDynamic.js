    ...
    allProviders.forEach((provider, index) => {
        ...
        if (provider.importComponent === 'TIME_OF_USE_IMPORT') {
            importHTML = `<h4>Import Rates (Time of Use)</h4>
                ...
                <label>Peak Hours: <input type="text" class="provider-input" data-id="${provider.id}" data-field="peakHours" value="${sanitize(provider.peakHours || '')}" placeholder="e.g., 7am-10am, 4pm-10pm"></label>
                <label>Shoulder Hours: <input type="text" class="provider-input" data-id="${provider.id}" data-field="shoulderHours" value="${sanitize(provider.shoulderHours || '')}" placeholder="e.g., 10am-4pm"></label>
                <label>Off-Peak Hours: <input type="text" class="provider-input" data-id="${provider.id}" data-field="offPeakHours" value="${sanitize(provider.offPeakHours || '')}" placeholder="e.g., 10pm-7am"></label>`;
        ...
        if (provider.exportComponent === 'MULTI_TIER_FIT') {
            exportHTML = `<h4>Export Rates (Tiered)</h4>
        ...
        providersHTML += `<details class="collapsible-section provider-details" open data-provider-id="${provider.id}">
                <summary>
                ...
                <div class="subsettings">
                    ...
                    <details class="collapsible-section" open>
                        ...
                        <div class="subsettings">
                            ...
                            <label>Charge Start Hour (e.g., 23 for 11 PM): <input type="number" class="provider-input" data-id="${provider.id}" data-field="gridChargeStart" min="0" max="23" value="${provider.gridChargeStart || 0}"></label>
                            <label>Charge End Hour (e.g., 5 for 5 AM): <input type="number" class="provider-input" data-id="${provider.id}" data-field="gridChargeEnd" min="0" max="23" value="${provider.gridChargeEnd || 0}"></label>
                        </div>
                    </details>
                    <hr>
