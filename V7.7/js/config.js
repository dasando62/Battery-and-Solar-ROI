...
export function gatherConfigFromUI() {
    ...
    for (const pKey in providers) {
        ...
        if (pData.exportComponent === 'MULTI_TIER_FIT') {
            exportRule.tiers = [
                { limit: getNumericInput(`${pId}Export1Limit`), rate: getNumericInput(`${pId}Export1Rate`) },
                { limit: Infinity, rate: getNumericInput(`${pId}Export2Rate`) }
            ];
        ...
        const gridCharge = {
            enabled: document.getElementById(`${pId}GridChargeEnable`)?.checked,
            startTime: getNumericInput(`${pId}GridChargeStartTime`, 22),
            endTime: document.getElementById(`${pId}GridChargeEndTime`)?.value.trim() || "Threshold"
        };
        ...
        if (pKey === "GloBird") {
            config.providers[pKey].exportData = {
                bonusRate: getNumericInput(`${pId}SuperExportRate`),
                bonusLimit: getNumericInput(`${pId}SuperExportLimit`),
                touRates: [ { name: "4pm-9pm", hours: [16, 17, 18, 19, 20, 21, 22], rate: getNumericInput(`${pId}Export4pm9pmRate`) }, { name: "10am-2pm", hours: [10, 11, 12, 13, 14], rate: getNumericInput(`${pId}Export10am2pmRate`) }, { name: "Other", hours: [], rate: getNumericInput(`${pId}Export9pm10am2pm4pmRate`) } ]
            };
        } else {
